run = ->
  ...
  if program.args[0] == 'vendor'
    # exec 'mkdir -p ~/lib/vendor && cd ~/lib/vendor && bower i moment', (se, so, e)->
    exec 'cd '+ __dirname + ' && export bi=$(npm bin)/bower-installer && cd ~/lib/vendor && echo $bi && echo cool', (se, so, e)->
      console.log so
      process.exit()
  else
    mimes =
